...
function addHomeworkSs(studentData) {
    ...
    if (!homeworkSsId) {
      ...
      if (!studentData.studentFolderId) {
        const adminSubfolders = DriveApp.getFolderById(studentData.folderId).getFolders();
        while (adminSubfolders.hasNext()) {
          const adminSubfolder = adminSubfolders.next();
          if (adminSubfolder.getName().toLowerCase().includes(studentData.name.toLowerCase())) {
            studentData.studentFolderId = adminSubfolder.getId();
          }
        }
      }

      newHomeworkFile.moveTo(DriveApp.getFolderById(studentData.studentFolderId));
...
//       return;
//     }
//     studentData.name = prompt.getResponseText();
//   }
...
//   const existingStudentData = allStudentsData.find((student) => String(student.name).toLowerCase() === studentData.name.toLowerCase());
